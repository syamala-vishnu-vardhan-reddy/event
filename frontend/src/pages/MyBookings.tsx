      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {data?.map((booking: any) => (
          <div key={booking._id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
            ...
            <div className="p-6">
              <h3 className="text-xl font-semibold mb-2 text-gray-800">{booking.event.title}</h3>
              <p className="text-gray-600 text-sm mb-3">{booking.event.description}</p>
              
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-500">Date:</span>
                  <span className="font-medium">{new Date(booking.event.date).toLocaleDateString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Time:</span>
                  <span className="font-medium">{new Date(booking.event.date).toLocaleTimeString()}</span>
                </div>
                {booking.event.location && (
                  <div className="flex justify-between">
                    <span className="text-gray-500">Location:</span>
                    <span className="font-medium">{booking.event.location}</span>
                  </div>
                )}
                <div className="flex justify-between">
                  <span className="text-gray-500">Seats Booked:</span>
                  <span className="font-medium text-blue-600">{booking.seats}</span>
                </div>
                {booking.event.price > 0 && (
                  <div className="flex justify-between">
                    <span className="text-gray-500">Total Price:</span>
                    <span className="font-medium text-green-600">${(booking.event.price * booking.seats).toFixed(2)}</span>
                  </div>
                )}
                <div className="flex justify-between">
